...
const Progress = () => {
  ...
  const getWeeklyStats = (): WeekStats[] => {
    ...
    weeklyMap.forEach(weekStats => {
      ...
      const uniqueDays = new Set(weekSessions.map(session => new Date(session.date).toDateString()));
      weekStats.daysActive = uniqueDays.size;
      weekStats.avgDailyDuration = weekStats.daysActive > 0 ? Math.round(weekStats.totalDuration / weekStats.daysActive) : 0;
    });
    return Array.from(weeklyMap.values()).sort((a, b) => new Date(b.weekStart).getTime() - new Date(a.weekStart).getTime());
  };
  ...
  const getAchievements = () => {
    ...
    if (totalSessions >= 10) achievements.push({
      title: "Dedicated Breather",
      description: "10 sessions completed"
    });
    if (totalSessions >= 50) achievements.push({
      title: "Breathing Expert",
      description: "50 sessions completed"
    });
    if (totalTime >= 3600) achievements.push({
      title: "Hour of Zen",
      description: "1 hour of practice"
    });
    return achievements;
  };
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 pb-20">
      <div className="container mx-auto px-4 py-8 max-w-md">
        ...
        {/* Achievements */}
        {achievements.length > 0 && <Card className="mb-8 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-0 shadow-lg">
            <CardHeader>
        ...
        <Card className="mb-8 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-0 shadow-lg">
          ...
          <CardContent className="p-4 pt-0">
            <Tabs defaultValue="recent" className="w-full">
              <TabsList className="grid w-full grid-cols-3 bg-cyan-950">
              ...
              <TabsContent value="recent" className="space-y-3 mt-4">
                {sessions.length === 0 ? <div className="text-center py-8">
                  ...
                  </div> : sessions.slice(0, 10).map(session => <div key={session.id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                      <div>
              ...
              <TabsContent value="daily" className="space-y-3 mt-4">
                {dailyStats.length === 0 ? <div className="text-center py-8">
                    <p className="text-slate-600 dark:text-slate-400">No daily data available</p>
                  </div> : dailyStats.slice(0, 14).map(day => <div key={day.date} className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-700 last:border-b-0">
                      <div>
              ...
              <TabsContent value="weekly" className="space-y-3 mt-4">
                {weeklyStats.length === 0 ? <div className="text-center py-8">
                    <p className="text-slate-600 dark:text-slate-400">No weekly data available</p>
                  </div> : weeklyStats.slice(0, 8).map((week, index) => <div key={index} className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
